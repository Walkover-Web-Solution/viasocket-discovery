    ...
    <section className="py-5">
        ...
        {FAQs?.map((faq, index) => (
          <details className="border-top" style={{borderColor: 'var(--grey)'}} key={index}>
            <summary className="d-flex align-items-center justify-content-between gap-3 py-3 cursor-pointer list-none fw-semibold" style={{fontSize: '17px', color: 'var(--foreground-color)', userSelect: 'none'}}>
              {faq.question}
            ...
            <div className="pb-3">
              <p className="m-0" style={{fontSize: '15px', lineHeight: '1.65', color: 'var(--dark-grey)'}}>{faq.answer}</p>
            </div>
          </details>
        ))}
    </section>
