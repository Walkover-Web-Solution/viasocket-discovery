...
export default function Search({
  ...
  const handleClick = () => {
    ...
    if (searchQuery.trim() && allApps.length > 0) {
      const app = allApps.find(app => app.name.toLowerCase() === searchQuery.trim().toLowerCase());
      if (app) {
      ...
      <div className={`${styles.postHeader} ${className}`}>
        <div className={styles.searchBox}>
          ...
          <input
            ...
            placeholder={selectedApps.length > 0 ? "" : (placeholder || "Ask AI...")}
            value={searchQuery}
            ...
            onKeyDown={(e) => {
              ...
              if (e.key === "Backspace" && searchQuery === "" && selectedApps.length > 0) {
