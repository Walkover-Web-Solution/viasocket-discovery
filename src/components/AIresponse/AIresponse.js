...
const AIresponse = ({
  ...
  blogData?.blog?.forEach((section) => {
    ...
    if (section?.section === "FAQ") faqSection = section;
  ...
  useEffect(() => {
    const dr = document
      .getElementById("detailed-reviews")
  ...
  return (
    <>
      <Head>
        <title>
          {(blogData?.meta?.headerTitle || blogData?.title || "New chat") +
            " | Viasocket"}
        </title>
      </Head>
      <div
      ...
      >
        {Components["title"]({
          ...
          subHeading: blogData?.titleDescription,
            ...
            <div className={styles.floaterDiv}>
              ...
              >
                {Components["summaryList"]({
                  appNames: detailedReviews.content.map((app) => app.appName),
                  integrations,
                  meta: blogData.meta,
                })}
              </Box>
